...
def load_settings(
    ...
    mistral_keys_master_key = (
        values.get("LLM_KEYS_MASTER_KEY", "").strip()
        or values.get("MISTRAL_KEYS_MASTER_KEY", "").strip()
    )
    ...
    if llm_provider == "ollama":
        ...
        if not mistral_keys_master_key:
            ...
            errors.append("MISTRAL_KEYS_MASTER_KEY must be a valid Fernet key")
        # OPENAI_COMPATIBLE_API_KEY is optional after the first legacy import into the pool.
